...
| 이미지, 디자인, 시각화 | — **design-expert 사용 (아래 참조)** | **design-expert** |
...
## ★ 시각 디자인 태스크 — design-system.md 참조 필수

**시각 디자인 태스크는 일반 expert처럼 에이전트를 생성하되, 반드시 `docs/design-system.md`를 참조하도록 시스템 프롬프트에 명시합니다.**

`docs/design-system.md`는 타이포그래피, 컬러, 레이아웃, Self-Check Checklist 등 공유 디자인 규칙을 담고 있습니다. 이 파일을 읽지 않으면 폰트 크기·대비·여백 등의 품질 기준을 만족하기 어렵습니다.

### 시각 디자인 태스크 판별 기준

아래 키워드가 태스크명/설명에 포함되면 **시각 디자인 에이전트**로 분류합니다:

```
카드뉴스, 카드 이미지, SNS 이미지, 인스타그램 이미지
포스터, 배너, 썸네일, 커버 이미지
슬라이드 이미지, 프레젠테이션 슬라이드
인포그래픽, 시각 자료, 비주얼 콘텐츠
UI 목업, 앱 스크린샷, 프로토타입 이미지
```

### 시각 디자인 에이전트 생성 방법

일반 expert와 동일하게 `hired_agents.json`에 추가하고 `.claude/agents/{agent_id}.md` 파일을 생성합니다. 단, 아래 두 가지를 반드시 추가합니다:

**1. hired_agents.json — `is_visual_designer: true` 추가:**
```json
{
  "agent_id": "expert-00N",
  "role_name": "[디자인 역할명]",
  "capabilities": ["visual_design", "card_news", "html_to_png"],
  "feasible_tools": ["Bash", "Write", "Edit", "Read"],
  "base_tools": ["Read", "Write", "Edit", "Bash", "WebSearch", "WebFetch", "Glob", "Grep"],
  "assigned_tasks": ["task-XXX"],
  "status": "active",
  "is_visual_designer": true
}
```

**2. 에이전트 시스템 프롬프트 — `docs/design-system.md` 참조 섹션 추가:**

```markdown
## ★ 시각 디자인 규칙 (작업 전 필수)

**모든 시각 디자인 작업 시작 전에 `docs/design-system.md`를 반드시 읽습니다.**
이 파일에는 Layer A 절대 규칙, 컬러/타이포그래피 시스템, Design Brief 양식,
Self-Check Checklist가 포함되어 있습니다.

실행 순서:
1. `docs/design-system.md` 읽기 (전체)
2. Design Brief 작성 (코드 작성 전 필수)
3. HTML/CSS 구현 → Self-Check Checklist 점검
4. Chrome headless PNG 렌더링
5. 결과 저장 + execution_log 기록
```

### task_type 지정

시각 디자인 태스크는 반드시 `task_type: "ACTION_VISUAL"`로 지정합니다. QA가 `docs/design-system.md` 기준으로 검증합니다.

```json
{
  "task_id": "task-XXX",
  "task_type": "ACTION_VISUAL",
  "agent_id": "expert-00N",
  ...
}
```
